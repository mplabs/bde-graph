(function (context) {
  ...
  function findPointOnCubicBezier(p, sx, sy, c1x, c1y, c2x, c2y, ex, ey) {
    // p is ratio from 0 to 1
    var op = 1 - t;
    // 3 green points between 4 points that define the cure
    var g1x = sx * p + c1x * op;
    var g1y = sy * p + c1y * op;
    ...
    var g3x = c2x * p + ex * op;
    var g3y = c2y * p + ey * op;
    // 2 blue points between the green points
    var b1x = g1x * p + g2x * op;
    var b1y = g1y * p + g2y * op;
    var b2x = g2x * p + g3x * op;
    var b2y = g2y * p + g3y * op;
    // Point on the curve between the blue points
    var x = b1x * p + b2x * op;
    var y = b1y * p + b2y * op;
    return [x, y];
  }
  ...
  TheGraph.Edge = React.createFactory( React.createClass({
    ...
    componentDidMount: function() {
      ...
      if (this.props.onEdgeSelection) {
        // Needs to be clicked (not tapped) to get event.shiftKey
    ...
    render: function() {
      ...
      if (targetX - 5 < sourceX) {
        ...
        if (Math.abs( targetY - targetX) < TheGraph.config.nodeSize / 2) {
          // Loopback
      ...
      // Make SVG path
      var path = TheGraph.factories.edge.createEdgePathArray(
        sourceX, sourceY,
        c1X, c1Y,
        c2X, c2Y,
        targetX, targetY
      ...
      var backgroundPathOptions = TheGraph.merge(
        TheGraph.config.edge.backgroundPath,
        { d: path }
      );
      ...
      var foregroundPathOptions = TheGraph.merge(
        TheGraph.config.edge.foregroundPath,
        { d: path, className: foregroundPathClassName }
      );
      var foregroundPath = TheGraph.factories.edge.createEdgeForegroundPath(foregroundPathOptions);

      var touchPathOptions = TheGraph.merge(
        TheGraph.config.edge.touchPath,
        { d: path }
      );
      ...
      containerOptions = TheGraph.merge(
        TheGraph.config.edge.container,
        containerOptions
      );
      return TheGraph.factories.edge.createEdgeGroup(
        containerOptions,
        [backgroundPath, foregroundPath, touchPath]
      );
    }
  }));
