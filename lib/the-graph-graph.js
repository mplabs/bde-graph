(function(context) {
  ...
  TheGraph.Graph = React.createFactory( React.createClass({
    ...
    edgeStart: function(event) {
      ...
      // @todo: tap to add new nodes here
    ...
    renderPreviewEdge: function(event) {
      var x = event.detail.x || event.clientX || 0;
      var y = event.detail.y || event.clientY || 0;
    ...
    addEdge: function(edge) {
      this.state.graph.addEdge(
        edge.from.process, edge.from.port,
        edge.to.process, edge.to.port,
        edge.metadata
      );
    },
    ...
    setErrorNodes: function(nodes) {
      this.setState({ errorNodes: nodes });
      this.markDirty();
    },
    ...
    setAnimatedEdges: function(edges) {
      this.setState({ animatedEdges: edges });
      this.markDirty();
    },

    updateIcons: {},
    updateIcon: function(nodeId, icon) {
      this.updateIcons[nodeId] = icon;
      this.markDirty();
    },
    ...
    render: function() {
      ...
      var nodes = graph.nodes.map(function(node) {
        ...
        if (undefined === node.metadata.x) {
          node.metadata.x = 0;
        }
        if (undefined === node.metadata.y) {
          node.metadata.y = 0;
        }
        if (undefined === node.metadata.width) {
        ...
        var icon = "cog";
        var iconsvg = "";
        if (this.updateIcons[key]) {
          icon = this.updateIcons[key];
      ...
      // Inport exports
      // Floating external inslots may not have a private process and port
      // @todo (fdu) Make it so!
      var inports = Object.keys(graph.inports).map(function(key) {
        ...
        // @todo (fdu): Check wether this is ithe way to find our connected
        // node/port
        ...
        // Draw an edge, if we have a connection defined
        if (privateNode && privatePort) {
          // Edge view
          var expEdge = {
            key: "inport.edge." + key,
            export: inport,
      ...
      if (edgePreview) {
        // @todo (fdu) You need to make a switch here. Dropping edge previews on
        // ext-ports must be handled. At the moment this will try to find the
        // corresponding process, which will fail.
      ...
      var groupsOptions = TheGraph.merge(
        TheGraph.config.graph.groupsGroup,
        { children: groups }
      );
      var groupsGroup = TheGraph.factories.graph.createGraphGroupsGroup.call(this, groupsOptions);

      var edgesOptions = TheGraph.merge(
        TheGraph.config.graph.edgesGroup,
        { children: edges }
      );
      var edgesGroup = TheGraph.factories.graph.createGraphEdgesGroup.call(this, edgesOptions);

      var iipsOptions = TheGraph.merge(
        TheGraph.config.graph.iipsGroup,
        { children: iips }
      );
      var iipsGroup = TheGraph.factories.graph.createGraphIIPGroup.call(this, iipsOptions);

      var nodesOptions = TheGraph.merge(
        TheGraph.config.graph.nodesGroup,
        { children: nodes }
      );
